...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Hydro Planner v1.3 (Fixed)")
...
# --- B. HALAMAN KLIMATOLOGI (SUDAH DI-FIX) ---
    ...
    if st.button("🚀 HITUNG ETo SEKARANG", type="primary"):
        ...
        st.session_state['hasil_eto'] = df_res # Simpan ke session_state
        st.success("✅ Perhitungan Selesai & Disimpan!")

    # LOGIKA PERSISTENCE: Cek apakah hasil sudah ada di memori?
    if 'hasil_eto' in st.session_state:
        df_show = st.session_state['hasil_eto']
        
        # Tampilkan Hasil (akan tetap muncul meski pindah halaman)
        st.divider()
        c_res1, c_res2 = st.columns([1, 2])
        
        with c_res1:
            avg_eto = df_show['ETo (mm/hari)'].mean()
            st.info(f"**Rata-rata ETo:**\n# {avg_eto:.2f} mm/hari")
            st.dataframe(df_show, height=300, use_container_width=True)
            
        with c_res2:
            st.caption("Grafik ETo Tahunan")
            st.bar_chart(df_show.set_index('Periode')['ETo (mm/hari)'])
    else:
        st.info("👋 Silakan klik tombol 'HITUNG' untuk melihat hasil.")
...
# --- C. HALAMAN POLA TANAM & NFR ---
    ...
    if 'hasil_eto' in st.session_state:
        ...
        st.warning("⚠️ Data ETo belum dihitung di halaman Klimatologi. Menggunakan nilai default 4.5 mm/hari.")
