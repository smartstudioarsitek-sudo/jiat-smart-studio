...
<style>
    ...
    .save-card {padding: 20px; background-color: #f1f8e9; border-radius: 10px; border: 1px solid #c5e1a5;}
...
def serialize_session():
    ...
    for k, v in st.session_state.items():
        # Filter file uploader & widget internal
...
# --- BAGIAN 1: IDENTITAS PROYEK (INPUT DISINI) ---
st.markdown("### 1️⃣ Identitas Proyek")
with st.container():
    st.markdown('<div class="project-card">', unsafe_allow_html=True)
    c1, c2, c3 = st.columns([2, 2, 1])
    with c1:
        st.session_state['nama_proyek'] = st.text_input("Nama Pekerjaan", value=st.session_state['nama_proyek'])
    with c2:
        st.session_state['lokasi'] = st.text_input("Lokasi / Desa", value=st.session_state['lokasi'])
    with c3:
        st.session_state['tahun'] = st.number_input("Tahun Anggaran", value=st.session_state['tahun'])
...
with c_left:
    ...
    uploaded = st.file_uploader("Upload file .jiat atau .json", type=['json', 'jiat'])
    if uploaded:
        if st.button("📂 Load Project"):
            ok, msg = load_session(uploaded)
            if ok: 
                st.success(f"✅ Berhasil memuat {msg} data! Silakan cek halaman lain.")
                st.rerun()
            else: st.error(f"Gagal: {msg}")

# SAVE PROYEK
with c_right:
    st.markdown("### 💾 Simpan Proyek (Save As)")
    st.info("Tombol ini akan menyimpan SELURUH DATA dari semua halaman.")
    
    # Siapkan nama file otomatis
...
for i, (key, label) in enumerate(modules):
    with cols[i]:
        if key in st.session_state:
            st.success(f"✅ {label}: Ada")
        else:
            st.warning(f"⬜ {label}: Kosong")
