...
def hitung_eto(temp, hum, sun, wind, lat_deg=-7.0, alt=10):
    # Fungsi bantu untuk memastikan input jadi float array aman
    def to_float(arr):
    ...
    try:
        ...
        return np.zeros(12) # Return 0 jika error perhitungan
...
try:
    ...
    # Buat DataFrame Hasil (Hanya Bulan dan ETo)
    df_hasil = edited_df[['Bulan']].copy()
    df_hasil['ETo (mm/hari)'] = np.round(eto_result, 2)
    
    # --- [PENTING] SIMPAN DATA KE MEMORI APLIKASI ---
    ...
    with col1:
        # [SOLUSI ANTI-CRASH]:
        # Kita filter dulu: Mana kolom yang isinya ANGKA?
        numeric_cols = df_hasil.select_dtypes(include=[np.number]).columns
        
        # Terapkan format .2f HANYA pada kolom angka (subset=numeric_cols)
        # Kolom 'Bulan' aman tidak akan disentuh formatter
        st.dataframe(
            df_hasil.style
