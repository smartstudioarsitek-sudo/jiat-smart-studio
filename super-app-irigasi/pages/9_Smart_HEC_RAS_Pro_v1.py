...
def calculate_profiles(nodes, Q, boundary_down, boundary_up, force_super=False):
    ...
    for n in nodes:
        ...
        # Bank Elevation & Freeboard
        H_ch = n.get('h_ch', 1.5) # Default H=1.5m jika user tidak isi
...
with st.sidebar:
    ...
    st.divider()
    up_file = st.file_uploader("Upload Excel", type=['xlsx'], key="xls_rekap_v6")
    if up_file:
        try:
            ...
            if found>=5: 
                for r in REQUIRED_COLS: 
                    if r not in new_df.columns: 
                        if "Tinggi" in r: new_df[r] = 1.5 # Default H
                        else: new_df[r] = 0
...
with t1:
    st.caption("Pastikan mengisi kolom **'Tinggi Saluran H (m)'** agar perhitungan Tinggi Jagaan (Freeboard) akurat.")
...
with t2:
    if len(profile['x']) > 0:
        ...
        ax.plot(profile['x'], profile['bank'], 'brown', ls='--', lw=2, label='Bibir Tanggul (Bank)') # New
    ...
    if len(all_nodes) > 0:
        ...
        if node:
            ...
            with c1:
                ...
                H_draw = node['h_ch'] # Pakai Tinggi Saluran Asli
            ...
            with c2:
                ...
                st.metric("Tinggi Saluran Eksisting", f"{node['h_ch']:.2f} m")
...
with t4:
    if final_data:
        ...
        # Conditional Formatting Simpel (Highlight Jagaan Rendah)
